...
import math
import random
from models.encoder import Mix_TR
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0.1, context_dim=None, part='encoder', vocab_size=None):
        super().__init__()
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.mix_net = Mix_TR(nb_classes=nb_classes, d_model=context_dim)
        #==================== INPUT BLOCK ====================
    ...
    def forward(self, x, timesteps=None, style=None, content=None, wid=None, tag='test', **kwargs):
        """
        ...
        if tag=='train':
            context, vertical_proxy_loss, horizontal_proxy_loss = self.mix_net(style, content, wid)
        else:
        ...
        if self.predict_codebook_ids:
            ...
            if tag == 'train':
                return self.out(h), vertical_proxy_loss, horizontal_proxy_loss
            else:
                return self.out(h)